...
class Producto{
    constructor(pId, pNombre, pPrecio, pDestino){
        this.id = pId;
        this.nombre = pNombre;
        this.precio = pPrecio;
...
function cargarProducto(e){
    ...
    if(datosOk){
        ...
        if(confirma){            
            ...
            let productoActual = new Producto(contador, pNombre, pPrecio, pDestino); 
            contador++;
...
function mostrarTodos(){
    ...
    for(let item of listaDeProductos){
        switch(item.destino){
            case "Argentina":
                contenedor.innerHTML +=`<div class="item" id="${item.id}">
                                        <h3>${item.nombre}</h3>
                                        ...
                                        <button type="button" class="btn btn-primary borrarProducto" onClick="borrarProducto(${item.id})"><i class="fas fa-trash"></i></i></button>
                                        </div>`;
            break;
            case "Bolivia":
                contenedor.innerHTML +=`<div class="item" id="${item.id}">
...
function borrarProducto(id) {
    $(`#${id}`).hide();
    let objeto = listaDeProductos.find( producto => producto.id === id );
    let indiceObjeto = listaDeProductos.indexOf(objeto);
    listaDeProductos.splice(indiceObjeto, 1);
    comprobarVacio();
}  
